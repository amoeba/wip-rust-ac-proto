...
impl crate::writers::ACWritable for TradeTradeFailure {
    fn write(&self, writer: &mut dyn ACWriter) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "tracing")]
        let _span = tracing::span!(tracing::Level::DEBUG, "write", r#type = "TradeTradeFailure").entered();

        self.object_id.write(writer)?;
        write_u32(writer, self.reason)?;
        Ok(())
    }
}
